...
The Reveal.js deck runs 33 slides in four parts — one per thing you have to get right — with facilitator notes, two concept checks, and nine browser-based labs.
...
- **Lab 6 — sensitivity (Q3, Q4).** Sum versus average, with the neighbouring distribution drawn alongside so `Δf` is literally the gap between the two curves. Sliding `n` leaves a sum's noise untouched and collapses an average's. The Q4 toggle doubles `Δf` to `2h` while only one record changed.
